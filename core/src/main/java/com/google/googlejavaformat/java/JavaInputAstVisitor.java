...
public final class JavaInputAstVisitor extends TreePathScanner<Void, Void> {
    ...
    public Void visitMethod(MethodTree node, Void unused) {
        ...
        builder.open(ZERO);
        {
            if (!node.getParameters().isEmpty() || node.getReceiverParameter() != null) {
                ...
                builder.breakToFill("");
                visitFormals(
                        Optional.fromNullable(node.getReceiverParameter()), node.getParameters());
            }
            token(")");
            if (dims != null) {
                maybeAddDims(dims);
        ...
        if (node.getDirectives().isEmpty()) {
            ...
            for (DirectiveTree directiveTree : node.getDirectives()) {
                ...
                builder.blankLineWanted(
                        previousDirective
                                        .transform(k -> !k.equals(directiveTree.getKind()))
                                        .or(false)
                                ? BlankLineWanted.YES
                                : BlankLineWanted.NO);
                builder.forcedBreak();
     ...
     * currently true if there is at most one marker annotation, and no others.
     *
